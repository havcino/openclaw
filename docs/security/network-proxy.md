...
- Gateway loopback control-plane traffic defaults to direct local bypass through `proxy.loopbackMode: "gateway-only"`. The bypass requires the actual connection URL to match the configured Gateway URL, so custom loopback ports still work without opening arbitrary loopback destinations. Operators can set `proxy.loopbackMode: "proxy"` to send the configured Gateway loopback URL through the managed proxy, or `proxy.loopbackMode: "block"` to deny loopback Gateway connections. If loopback traffic is sent through a remote proxy, the proxy must provide special routing for the OpenClaw host's loopback service, such as mapping it to a proxy-reachable hostname, IP, or tunnel. Standard remote proxies resolve `127.0.0.1` and `localhost` from the proxy host, not from the OpenClaw host, so proxied loopback traffic will otherwise fail or reach the wrong machine.
